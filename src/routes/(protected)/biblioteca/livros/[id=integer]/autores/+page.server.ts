...
export const load: PageServerLoad = async ({ locals, params }) => {
	...
	try {
		...
		const autoresLivro = await db.select({ idautor: autor.idautor, nome: autor.nome }).from(autor_has_livro)
			.innerJoin(autor, eq(autor_has_livro.autor, autor.idautor))
			.where(eq(autor_has_livro.livro, Number(params.id))).orderBy(autor.nome);
...
export const actions: Actions = {
	...
	excluir: async ({ url, params }) => {
		...
		if (!autor) {
			return fail(400, { message: 'Nenhum autor foi selecionada para exclusão' });
		}

		try {
			await db.delete(autor_has_livro).where(and(eq(autor_has_livro.livro, livro), eq(autor_has_livro.autor, Number(autor))));
			return { status: 200 };
		} catch (err) {
			console.log(err)
